...
export async function createCursor(page: playwright.Page, createCursorOptions?: createCursorOptions) {
  // defaults
...
export class Cursor {
  ...
  async getRandomPointOnViewport(paddingPercentage = 0): Promise<Vector> {
    const windowBoundaryBox = JSON.parse(
      await this.page.evaluate(() => JSON.stringify({ width: window.innerWidth, height: window.innerHeight }))
    )
  ...
  async getActualPosOfMouse(): Promise<Vector> {
    const actualPos = JSON.parse(await this.page.evaluate(() => JSON.stringify(window['mousePos']))) as Vector
    return actualPos
  }
  ...
  actions: Actions = {
    click: async (clickOptions: clickOptions, moveOptions: moveOptions): Promise<void> => {
      ...
      // utils
      const justClick = async (waitBetweenClick = [20, 50] as [number, number], doubleClick = false): Promise<void> => {
